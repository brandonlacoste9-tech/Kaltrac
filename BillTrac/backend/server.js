...
app.get('/api/analytics/predictive', authenticateToken, requireSecureSession, async (req, res) => {
  ...
  try {
    ...
    // 3. Execute Deep Insight (Local AI)
    const prompt = `Perform a FISCALOOK DEEP INSIGHT audit on this financial dataset:
    ...
    const response = await ollama.generate({
      model: 'llama3.1:8b', // Using the reasoning engine
      prompt: prompt,
      format: 'json',
      stream: false
    });

    const insight = JSON.parse(response.response);
    res.json(insight);
