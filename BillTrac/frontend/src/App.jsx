...
const App = () => {
  ...
  const handleRegionSelect = (selectedRegion, selectedRole) => {
    const updatedUser = { 
      ...user,
      name: user?.name || (selectedRole === 'BOSS' ? "Director Lacoste" : "Field Agent"), 
      role: selectedRole 
    };
                ...
                {authMode === 'register' && (
                  <div>
                    ...
                    <input 
                      ...
                      placeholder="e.g. Director Lacoste"
                    />
                  </div>
                )}
                <div>
          ...
          <div className="md:w-1/2 p-12 bg-black/40 border-r border-white/5 flex flex-col justify-center">
            ...
            <div className="space-y-4">
              <button 
                ...
                className={`w-full p-6 leather-card border-none text-left transition-all ${role === 'BOSS' ? 'ring-2 ring-gold bg-white/5' : 'opacity-40 hover:opacity-100'}`}
              >
                <div className="font-bold text-2xl gold-heading" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Executive Vault</div>
                <div className="gold-label mt-1">Full Auditing • Tax Recovery • Organization</div>
              </button>
              
              <button 
                onClick={() => setRole('USER')}
                className={`w-full p-6 leather-card border-none text-left transition-all ${role === 'USER' ? 'ring-2 ring-gold bg-white/5' : 'opacity-40 hover:opacity-100'}`}
              >
                <div className="font-bold text-2xl gold-heading opacity-50 hover:opacity-100 transition-opacity" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Staff Entrance</div>
                <div className="gold-label mt-1 opacity-40">Simple Scan • Immediate Deposit</div>
              </button>
            </div>
          </div>
               ...
               {!role ? (
                ...
                >
                  <p className="gold-label opacity-30 text-center tracking-[0.4em] text-[9px] mb-8">
                    PROTOCOL: {role === 'BOSS' ? 'EXECUTIVE' : 'STAFF'} • {country}
                  </p>
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-12">
            ...
            <div className={`leather-card ${role === 'USER' ? 'hidden' : ''}`}>
          ...
          <div className={`leather-card ${role === 'USER' ? 'border-gold/40 shadow-[0_0_30px_rgba(212,175,55,0.1)]' : ''}`}>
            <h2 className="text-lg font-bold mb-6 flex items-center gap-3 uppercase tracking-widest text-gold/80">
              <Camera size={18} /> {role === 'USER' ? 'Urgent Submission' : t('DOCUMENT SCAN')}
            </h2>
              ...
              <label htmlFor="receipt-upload" className="file-input group">
                <div className="text-center">
                  ...
                  <p className="gold-label opacity-40 group-hover:opacity-100 transition-opacity">
                    {role === 'USER' ? 'Drop Receipt for Approval' : t('DEPOSIT DIGITAL VOUCHER')}
                  </p>
                </div>
              </label>
            </div>
